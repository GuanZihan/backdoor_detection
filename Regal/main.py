...
dataset, injected_graph_idx, backdoor_dataset, clean_dataset = inject_sub_trigger(dataset)

nx_dataset = to_nx_graph(dataset)

accs = []
for idx in range(0, len(nx_dataset), 2):
    # print(nx_dataset[idx])
    # print(nx_dataset[idx+1])
    combined_graph = combine_graphs(nx_dataset[idx], nx_dataset[idx+1])
    np.random.seed(2021)
    random_select_nodes_1 = np.random.choice(len(nx_dataset[idx].nodes), 6, replace=False)
    # print(random_select_nodes_1)
    np.random.seed(2021)
    random_select_nodes_2 = np.random.choice(len(nx_dataset[idx+1].nodes), 6, replace=False)
    # print(random_select_nodes_2)

    color_map_1 = []
    for node in nx_dataset[idx]:
        if node in random_select_nodes_1:
            color_map_1.append('bisque')
        else:
            color_map_1.append('lightblue')

    color_map_2= []
    for node in nx_dataset[idx+1]:
        if node in random_select_nodes_2:
            color_map_2.append('orange')
        else:
            color_map_2.append('blue')

    pos = nx.spring_layout(nx_dataset[idx], k=0.5,scale=3)
    nx.draw(nx_dataset[idx],pos, node_color=color_map_1, with_labels = True)
    pos = nx.spring_layout(nx_dataset[idx+1],  k=0.5,scale=3)
    nx.draw(nx_dataset[idx+1],pos, node_color=color_map_2,  with_labels = True)

    plt.show()
    embed = learn_representations(combined_graph, args)
    emb1, emb2 = get_embeddings(embed, len(nx_dataset[idx].nodes))
    if args.numtop == 0:
        args.numtop = None
    alignment_matrix = get_embedding_similarities(emb1, emb2, num_top=None)  # args.numtop)
    # print(alignment_matrix)
    plt.imshow(alignment_matrix)
    plt.show()
    true_alignments = []
    count = 0

    for idx, node in enumerate(random_select_nodes_2):
        true_alignments.append([node, random_select_nodes_1[idx]])
    for cor in true_alignments:
        x, y = cor[0], cor[1]
        error = np.max(alignment_matrix[y]) - alignment_matrix[y][x]
        count += np.max(alignment_matrix[y]) == alignment_matrix[y][x]
        print(np.max(alignment_matrix[y]) , alignment_matrix[y][x])
        print(count)
    accs.append(count / len(true_alignments))
